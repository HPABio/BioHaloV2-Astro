...
// Handle potential module resolution differences (SSR vs Client)
...
const fragmentShader = `
  ...
  void main() {
    ...
    // We use the user color to tint the result
    vec3 mixedColor = color.rgb * uColor;
...
const CPPNShaderMaterial = shaderMaterial(
  { iTime: 0, iResolution: new THREE.Vector2(1, 1), uColor: new THREE.Color(1, 1, 1) },
  vertexShader,
  fragmentShader
);
...
function ShaderBackground({ color }: { color: string }) {
...
export default function Hero({ color }: { color: string }) {
  ...
  return (
    <section className="relative h-screen w-screen overflow-hidden">
      <ShaderBackground color={color} />

      {/* <div className="absolute top-4 right-4 z-50 flex items-center gap-2 bg-black/50 p-2 rounded-lg backdrop-blur-sm border border-white/10">
        <label htmlFor="color-picker" className="text-white text-sm font-light">Tint Color:</label>
        <input
          type="color"
          id="color-picker"
          value={color}
