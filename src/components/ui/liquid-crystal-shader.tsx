...
float mapScene(vec2 uv) {
  ...
  vec2 bounds = vec2(0.5 * aspect, 0.5);
  float margin = 0.05;

  // Sphere 1
  vec2 max1 = max(vec2(0.0), bounds - u_radii.x - margin);
  vec2 amp1 = min(vec2(0.3), max1);
  vec2 p1 = vec2(cos(t * 0.5), sin(t * 0.5)) * amp1;

  // Sphere 2
  vec2 max2 = max(vec2(0.0), bounds - u_radii.y - margin);
  vec2 amp2 = min(vec2(0.4), max2);
  vec2 p2 = vec2(cos(t * 0.7 + 2.1), sin(t * 0.6 + 2.1)) * amp2;

  // Sphere 3
  vec2 max3 = max(vec2(0.0), bounds - u_radii.z - margin);
  vec2 amp3 = min(vec2(0.35), max3);
  vec2 p3 = vec2(cos(t * 0.4 + 4.2), sin(t * 0.8 + 4.2)) * amp3;
